...
export function ResourceCard({ resource, index, showCategory = true }: ResourceCardProps) {
  const categoryColor = CATEGORIES_LIST.find((c) => c.name === resource?.category)?.color || "from-gray-500 to-slate-500"
  ...
  return (
    ...
    >
      <Card className="cursor-pointer h-full border hover:border-primary/50 transition-all duration-300 relative overflow-hidden backdrop-blur-sm group-hover:-translate-y-1 group-hover:shadow-xl group-hover:shadow-primary/20 rounded-sm">
        {/* Gradient top border */}
        <div
